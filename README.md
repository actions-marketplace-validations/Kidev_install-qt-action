...
**This fork essentially adds a way to pass the new `--wasm [none/singlethread/multithread]` parameter to [kidev's `aqtinstall fork`](https://github.com/Kidev/aqtinstall)**
...
This creates the version 4.2.\* of the Action as backward compatibility is maintained.

#### Notable changes to support WASM for Qt 6.7+

Backward compatibility is maintained, but in order to have WASM working with Qt 6.7+, you must:

- Select a host arch like usual, for example `gcc_64`
- Change `aqtversion` to `==3.2.*`
- Add (only for Qt 6.7+) the parameter `wasm` that can be `none` (default), `singlethread`, `multithread`

  ```yml
  - name: Install Qt for host & target architecture
  uses: Kidev/install-qt-action@v4.2.2
  with:
    aqtversion: '==3.2.*'
    version: '6.8.0'
    host: 'linux'
...
Downloads the specified WASM version as well. You must still gives an `arch` for the host (like `gcc_64`)
